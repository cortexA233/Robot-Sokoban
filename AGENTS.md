...
- Keep future requirements and their concrete version assignments in the [iteration roadmap](Docs/Versions/Roadmap.md). When the user requests a multi-version schedule, register the planned versions and create their scope, dependencies, exclusions and acceptance plans immediately. Roadmap IDs remain stable requirement references; planning does not start implementation.
- Before starting any iteration, including documentation/configuration maintenance, read the [version ledger](Docs/Versions/README.md). Use the already assigned planned version for scheduled work; do not allocate a later number just because future versions are reserved. For unscheduled independent work, choose an unused `vMAJOR.MINOR.PATCH` under the ledger rules and create its plan. Record planned status first, then the actual implementation starting commit and in-progress status before implementation. Do this autonomously; ask only for missing decisions that materially affect scope.
