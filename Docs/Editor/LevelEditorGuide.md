...
这是可用的制作与试玩基础版本，不是 GDD 第 6 节 E01–E12 的完整验收版本。正式目录资源和游戏内选关已经接入，编辑器窗口中的目录管理仍待实现；第一次编辑建议直接打开左侧的 L04 或 LAB01。
...
编辑器内目录管理、进度存档、反馈和完整界面验收见 [后续范围](../ImplementationProgress.md)；游戏菜单与选关UI的当前实现见 [UIImplementation.md](../UI/UIImplementation.md)。
