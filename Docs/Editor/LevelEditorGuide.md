...
窗口按文件、编辑、校验与试玩、目录与帮助分组，现场编辑工具默认折叠。左侧画笔分为工具、地形、角色与箱子、机关；示例关卡在可展开的“示例关卡”中。第一次编辑可打开 L04 或 LAB01。当前版本的验收范围见 [v0.4.0 计划与结果](../Versions/V0.4.0.md)，完整交付范围另见 GDD 第 6/11 节。
...
1. 点击“新建”。默认地图 9×9；展开右侧“关卡属性与尺寸”，设置宽高（5–32）并点击“应用尺寸”。点击“外围墙”可封闭边界。
...
- 顶部状态条持续显示选择/画笔模式、右键擦除层和放置朝向；当前画笔带勾选标记。按 Esc 或点击左侧“选择”返回选择模式。
...
- 坐标、箱型、朝向和“删除选中元素”作用于当前对象。移动箱子不带走下面的插槽；右键擦除仍由擦除层决定。放置模式允许连续绘制，不会因为查看属性而自动放置新对象。

## 添加固定转向板

1. 选择左侧“固定转向板”，在“朝向”选择 N/E/S/W，放在没有插槽或门的普通地板上。SceneView 悬停显示放置方向。
2. 选中后在右侧修改“朝向”，或按 Q/E 旋转。箱子覆盖板时，二维格子仍显示方向；游戏内板边有方向标记。
3. 可以连接低摩擦轨道，但板与轨道不能占同一格。两类箱子入板后沿箭头继续，机器人正常行走；受阻停稳后不会自行重启，需再次推动。
4. 先试左侧 L07/L08，再查看 L09–L12。修改方向后重新录制或回放解法；保存、另存副本、草稿恢复、缩图和现场编辑均保留板方向。

## 编辑与工作保护

- 左键选择/放置；地形支持连续拖刷。右键擦除当前“擦除层”：Terrain 为恢复地板，Devices 删除插槽/门/转向板，Actors 删除玩家/箱子。需要虚空时使用 Void 画笔。
...
2. 点击“使用当前作者关卡”，或将关卡 JSON 拖入“加入关卡 JSON”字段，然后“加入目录”。未保存、结构不合法、重复 ID、参考解法缺失/过期/回放失败时，加入会被拒绝并显示原因。
...
- 新图使用 schemaVersion=3，箱型和转向板方向随保存/打开、副本和草稿恢复保留。旧版关卡按能源箱读取，加入普通箱时自动升级；改箱型会使原参考解法过期，需要重新回放。
