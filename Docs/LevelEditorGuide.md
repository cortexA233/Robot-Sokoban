...
后续计划：[GM 与关卡编辑器现场试玩迭代方案](GMAndLiveEditingPlan.md)，包含可撤销 GM 移位，以及 Play Mode 内捕获、编辑临时局面并继续试玩。该方案尚未实现；下文仍描述当前可用流程。
...
编辑器试玩不保存正式玩家进度，也不显示“下一关”或运行时选关，以便保持作者副本与正式流程隔离。直接打开 Bootstrap Play 时，游戏读取 CampaignCatalog，支持通关后下一关、返回选关与暂停选关。目录资源在 `Assets/Resources/configs/CampaignCatalog.asset`，当前顺序为 L01→L02→L03。
