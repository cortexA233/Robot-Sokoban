...
模型、三段动画、URP材质、控制器和预制体已经交付。资产维护入口为 [sokoban-robot-asset](../.agents/skills/sokoban-robot-asset/SKILL.md)：几何/轴心用第2–4节，动画用第5节，导入导出用第6–7节，变更复验用第8节。游戏侧代码接入使用 [GameDesign.md](GameDesign.md) 第9节，不要求读取整个建模流程。
...
- Unity导入使用 [UnityMcp.md](UnityMcp.md) 的项目路由；资源路径和GUID规则见 [ProjectStructure.md](ProjectStructure.md)。
...
上述导入目录遵循 [ProjectStructure.md](ProjectStructure.md)；源文件与资产流水线仍保留在 `ArtSource/Robot/`。
