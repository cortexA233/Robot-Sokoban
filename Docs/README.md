...
[v0.11.0 美术细化实验](Versions/V0.11.0.md) 在当前目录的实验分支制作机器人、箱子、门、插槽与运输地面，并统一材质及相机效果；明确排除悬空平台收边。当前状态与验证见版本计划。

当前维护文档按用途归类，历史方案、实现记录和验收证据按开发阶段归档。先看 [当前实现状态与剩余范围](ImplementationProgress.md)，再按下面两种入口查找。

启动与试玩入口见 [项目首页](../README.md)。

[v0.8.2](Versions/V0.8.2.md) 将游戏正式名称统一为 **Robot Sokoban**，同步主菜单、产品配置、作者工具与文档；工程目录及历史验证证据保留原技术标识。

后续迭代先到 [开发版本台账](Versions/README.md) 规划版本号、范围和验收，再开始实现。当前追踪起点为 [v0.1.0 · 文档归类与版本管理](Versions/V0.1.0.md)。

后续工作原按 [迭代路线图](Versions/Roadmap.md) 分配到 v0.4.0–v0.9.0。[v0.4.0](Versions/V0.4.0.md)、[v0.5.0](Versions/V0.5.0.md) 及 [v0.6.0](Versions/V0.6.0.md) 已完成；[v0.7.0](Versions/V0.7.0.md) 已合入音效子项、整版进行中。2026-09-18 按用户要求取消未实施的 v0.8.0 引导/难度与 v0.9.0 完整交付验收；[v0.7.2](Versions/V0.7.2.md) 记录排期维护与 [F1 GM 使用说明](Editor/LevelEditorGuide.md#运行时-gm)。默认俯视、V 切换和连接配色的 [v0.5.0 验收与演示](Versions/V0.5.0Validation/README.md) 已记录；后续 [v0.5.1](Versions/V0.5.1.md) 按用户反馈取消第三人称局部剔除。[v0.4.1](Versions/V0.4.1.md) 补充了 v0.6.0 的 UI 简化要求：取消关卡标题、目标提示、完成文案配置及对应 UI。

用户指定追加的 [v0.4.1 编辑器简化](Versions/V0.4.1EditorSimplification.md) 也已完成：移除手动回放与擦除入口、修改工具文案并支持同层放置覆盖；实际范围和验收与原文档规划分开记录。

用户随后明确指定新的 [v0.8.0 收尾版本](Versions/V0.8.0Closeout.md)，已实施 R11：选关状态留空或仅显示“已完成 · N 步”，删除推动次数及“所有关卡均可选择”；追加 R12：删除已注释 MenuItem 对应的编辑器工具，已核对 10 项/9 个脚本及关联入口，保留在用资源与共享功能；追加 R13：GM 只留关卡/局面页，移除监视、诊断、动画与参考解法状态，局面页仅保留点格选对象和坐标移动，现场编辑入口自动打开编辑器并展开现场编辑组，两处文案按用户指定修改。已完成实现与自动/构建/原生现场草稿验证，实际结果与限制见版本计划；同号旧引导计划仍已取消，原范围不恢复。
...
[v0.7.1](Versions/V0.7.1.md) 已按用户要求独立完成：删除“从第一关开始”和门上浮动文字、第 9 关左上插槽四边统一橙色并保留双门供电。主菜单、电路回归与双分辨率 Player 检查见 [验收记录](Versions/V0.7.1Validation/README.md)；v0.7.0 余项仍保持进行中。
...
| 产品与技术规格 | GDD `1.10` | 文档修订号；完整验收目标与已经实现的范围分别看 GDD 和实现状态 |
