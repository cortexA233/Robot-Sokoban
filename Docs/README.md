...
用户随后明确指定新的 [v0.8.0 收尾版本](Versions/V0.8.0Closeout.md)，已规划 R11：选关状态留空或仅显示“已完成 · N 步”，删除推动次数及“所有关卡均可选择”；追加 R12：删除已注释 MenuItem 对应的编辑器工具，已核对 10 项/9 个脚本及关联入口，保留在用资源与共享功能；追加 R13：GM 只留关卡/局面页，移除监视、诊断、动画与参考解法状态，局面页仅保留点格选对象和坐标移动，现场编辑入口自动打开编辑器并展开现场编辑组，两处文案按用户指定修改。本次仅登记需求；同号旧引导计划仍已取消，原范围不恢复。
