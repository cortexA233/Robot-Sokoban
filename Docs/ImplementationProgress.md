...
1. **E10 正式关卡目录**：CampaignCatalog 资源、窗口加入/移出/排序、运行时目录读取与构建前全量回放门禁；当前只有作者关卡文件和开发场景入口。
2. **完整游戏流程**：主菜单、正式选关、下一关/最终结算、设置、持久化进度与最佳成绩对；开发版 HUD 需替换为正式 UI。
3. **编辑器验收**：从空白图通过实际鼠标键盘跑通第 11.4 节，检查连续拖刷、键盘焦点、关闭/取消、文件占用与域重载，逐项完成 E01–E12 记录。
