...
2026-09-16（America/New_York）完成首批 11 个资产的 `BoardView` 接入。[v0.2.0](../Versions/V0.2.0.md) 的文字与标牌方案保留历史证据；[v0.5.0](../Versions/V0.5.0.md) 更新为无插槽文字、按门配色和局部遮挡处理。
...
- `CameraOcclusion` 在 Cinemachine 更新前检查期望机位至机器人/相邻操作格的遮挡；第三人称局部隐藏墙和门的上层，恢复有 0.18 秒延迟与空间余量。墙和关闭门的 BoxCollider 仅用于镜头避障，隐藏时缩至 0.24 m 基座；逻辑占格与通行规则不变，箱子/插槽不添加物理驱动。
