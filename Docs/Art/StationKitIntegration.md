...
运行时按门分配蓝、橙、紫、黄、绿、粉连接色，同时配合圆、三角、菱形、方形、十字、双条符号。门与来源插槽的主关联共享身份材质，断电时保留；电力状态使用独立的填充标记。旧 `StationKitTheme.bindings` 与资产样式保留给隔离资产展示，正常游玩不生成来源文字标签。

- 目标环/外角标只由 `isGoal` 决定；插头只在该插槽被某扇门引用时显示。纯目标 B 没有插头，目标电源 A 同时显示环和插头，辅助电源 S 只有插头。
- `StationCircuitLayout` 按门 ID 排序分配颜色/图形索引，并按真实来源关系生成布线；输入数组重排、移动、撤销或相同数据重建不会改变身份。v0.7.1 起插槽四边统一为一个主关联的身份色：优先选择仅以该插槽为来源的门，并列或没有这种门时按稳定门 ID 排序选定。其余关联保留在真实连接表、按需线路和门的来源状态灯中，外框不再拼色；第七扇门起重复调色板并添加额外短条。
...
Any/All 继续来自现有 `GateDefinition.powerMode`；v0.7.1 移除门旁“任一/全部 · 通电数/总数”和占据保开文字，以及底框、引线。无供电却因占据保持开启时，门保留通行地标和琥珀色十字，来源灯继续独立显示。GM 按需诊断文字保留。
常态收起连接线，俯视指向或第三人称靠近门时显示该门所有分支，聚焦插槽时显示它到各门的分支。超大自定义网络的连线密度仍需专项易用性评估。

每个棋盘持有五份状态/底色材质、每门一份身份材质及复用 Mesh，状态切换复用它们，销毁棋盘时释放。材质使用主题显式引用的 URP/Unlit 裁切变体与白贴图，不通过 Shader.Find 猜测构建依赖。能源箱窗口和普通箱外观保持原状。
电路表现不再生成文字 Canvas 或标牌排版对象。连线查看直接使用门/插槽本体：俯视根据格子投影区域检测指针，第三人称根据与机器人距离、屏幕可见性和墙体遮挡检测附近设备；线路仍在 Cinemachine 更新后刷新。L12 的左上目标电源 socket_a 使用橙色 gate_F 的整圈边框，连接 gate_D 的蓝色线路仍保留。
