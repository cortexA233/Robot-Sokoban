...
版本：1.0 · 日期：2026-09-15 · 状态：可交付 Blender Agent
...
1. 读取现有 Blender 版本、当前文件名、场景对象与 Collection，确认是否已有同名资产或用户工作。
2. 本指南不固定 Blender 大版本；使用当前稳定可用版本，并把精确版本写入交付清单。不要为制作本资产擅自升级软件。
3. 创建专用 `Robot_Asset` Collection，参考物和相机放入独立 `Robot_Reference` Collection。
...
- 没有访问 Unity 时，`unityChecked=false`，并注明“等待 Unity Agent 导入验收”。不能把 Blender 回读当作 Unity 已验证。
...
1. **轮廓与尺寸**：先完成白模、推板接触和锚点。
2. **早期导出测试**：制作最小 Idle，导出回读，验证单位/朝向/物体动画可交付。
3. **机械结构与材质**：完成四类材质、轮组和推板细节。
4. **三段动画**：按规定时间轴制作，逐帧检查接触和循环。
5. **完整导出与预览**：导出FBX，回读，写实际 manifest 和验证结果。
6. **Unity 对接**：如可访问引擎则完成导入测试；否则交付明确的待验证项，由游戏 Agent 接入。

如时间不足，优先减少外壳小细节和装饰纹理，保留尺寸、锚点、三段动画、FBX及验证结果。不得通过缺少 Push 或移动轮组动画来缩减交付。

最终报告应列出：源文件与 FBX 路径、实际 Blender 版本、目标 Unity 版本、面数/材质统计、三段动画帧范围、锚点和接触测试结果、预览文件、Unity 验证是否完成及具体已知问题。
