...
namespace Sokoban.Tests
{
    public sealed class StationUITests
    {
        ...
        {
            runner.SelectLevel(0);
            var proof = JsonUtility.FromJson<SolutionRecord>(Resources.Load<TextAsset>("configs/solutions/L01.solution").text);
            ...
            Assert.That(Page<CompletionPage>().transform.Find("Content/Stats").GetComponent<Text>().text, Does.Contain("推动 3"));
        ...
        {
            runner.SelectLevel(0); runner.TryMove(Direction.S);
        ...
        [UnityTest] public IEnumerator HudUndoAndRestartUseTheRealSession()
        {
            ...
            Assert.That(runner.Session.State.Moves, Is.Zero);
            runner.TryMove(Direction.S); yield return new WaitForSecondsRealtime(.4f);
